---
name: criar-tasks
description: Converts PRD and Tech Spec into a detailed, sequenced list of implementation tasks for this NGO website. Each task is a functional, incremental frontend deliverable with matching verification steps. Outputs tasks.md and individual task files. Use when the user asks to create tasks, break down work, or plan implementation from an existing PRD and Tech Spec. Do not use for PRD creation, tech spec creation, or actual code implementation.
---
...
3. Order tasks logically: structure and content before polish, reusable foundations before dependent sections, and implementation before end-to-end verification.
4. Each task MUST be a functional, incremental deliverable.
5. Each task MUST have verification appropriate to the deliverable, such as lint/build validation, component tests, accessibility checks, or E2E coverage when relevant.
...
- Include validation as subtasks within each main task.
- Do NOT implement anything — focus solely on task listing and detailing.
- Prefer tasks that map to pages, sections, shared components, content updates, accessibility improvements, and SEO/performance checks.

## Quality Checklist
- [ ] PRD and Tech Spec analyzed.
- [ ] High-level task list approved by user.
- [ ] Task files generated using templates.
- [ ] Each task has appropriate validation subtasks.
